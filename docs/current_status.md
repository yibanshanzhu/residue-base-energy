...
| canonical PWM orientation | done | PWM 与所有 motif-slot supervision 同步变换 |
| full PWM MAE 的 per-sample 指标 | done | `rbe.eval.metrics` + `rbe.eval.summary` |
...
| PWM orientation | PWM/RC 二者展平后按字典序取较大者；所有 slot-axis labels 同步变换 |
| 单样本 MAE | 每个完整 PWM column 的四碱基 L1 之和，再对该 sample 的所有 columns 求均值 |
| 数据集汇总 | 先得到每个 sample 的指标，再对 samples 求均值 |
| `pwm_mask` | 只用于有结构依据的 contact/map 监督与评估，不参与 PWM 指标 |
| 其他 PWM 指标 | KL、IC-PCC 在 canonical 完整 PWM 上按 sample 计算 |

`canonical-pwm-orientation` 分支实现上述方向与评估口径，不包含 RC oracle 或跨样本 pooling columns。
